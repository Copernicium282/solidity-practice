...
contract TokenShop is Ownable {
    ...
    */
    ...
    function amountToMint(uint256 amountInETH) public view returns (uint256) {
        ...
        uint256 ethUsd = uint256(getChainlinkDataFeedLatestAnswer()) * 10 ** 10; // ETH/USD price with 8 decimal places default -> converting to 18 decimals
        uint256 ethAmountInUSD = amountInETH * ethUsd / 10 ** 18; // converting ETH to Wei (10^18 Wei = 1 ETH)
    ...
    receive() external payable {
        // convert the ETH amount to a token amount to mint automatically when a person sends ETH to this contract
        if (msg.value == 0) {
